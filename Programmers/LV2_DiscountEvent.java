...
public class LV2_DiscountEvent {
    public static void main(String[] args) {
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                String wanstArrayString = parts[0].substring(1, parts[0].length()-1); // want
                String[] wants = wanstArrayString.split(",");

                String numberArrayString = parts[1].substring(1, parts[1].length()-1); // number
                ...
                String discountArrayString = parts[2].substring(1, parts[2].length()-1); // discount
                String[] discounts = discountArrayString.split(",");

                int result = Integer.parseInt(parts[3].trim()); // result

                Integer myAnswer = solution(wants, numbers, discounts);
    ...
    public static int solution(String[] want, int[] number, String[] discount) {
        ...
        int wantCount = want.length;
        int discountDays = discount.length;
        int productsSum = Arrays.stream(number).sum();
        List<String> discountList = new ArrayList<>(Arrays.asList(discount));
        int days = 0;

        for(int i=0; i<discountDays-productsSum+1; i++) {
            List<String> discountSubList = new ArrayList<>(discountList.subList(i, i+productsSum));
            boolean flag = true;

            for(int j=0; j<wantCount; j++) {
                int discountCount = Collections.frequency(discountSubList, want[j]);
                if (number[j] > discountCount) {
                    flag = false;
                    break;
                }
            }
