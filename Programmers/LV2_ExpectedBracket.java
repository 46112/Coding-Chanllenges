...
class LV2_ExpectedBracket {
    ...
    public static int solution(int n, int a, int b) {
        ...
        int rivalNumber = b;
        int round = 0;
        boolean isGettingAlongWithRival = false;

        while (!isGettingAlongWithRival) {
            round++;
            myNumber = Math.floorDiv(myNumber+1, 2);
            rivalNumber = Math.floorDiv(rivalNumber+1, 2);

            isGettingAlongWithRival = (myNumber == rivalNumber);
        }
        return round;
    }
}
