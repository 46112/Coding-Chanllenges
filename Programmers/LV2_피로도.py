...
def solution(k, dungeons):
    answer = 0
    def dfs(depth, user_fatigue, dungeon_list):
        nonlocal answer
        for i, dungeon in enumerate(dungeon_list):
            if user_fatigue >= dungeon[0]:
                ds = dungeon_list.copy()
                ds.pop(i)
                count = dfs(depth+1, user_fatigue-dungeon[1], ds)
                if answer < count:
                    answer = count
        return depth
...
def validate(result_value, real_value):
    print("검증 시작!")
    assert result_value == real_value, f"정답: {real_value}, 유저 응답: {result_value}"
    print("검증 끝!")

result = solution(input_k, input_dungeons)
validate(result, output_result)